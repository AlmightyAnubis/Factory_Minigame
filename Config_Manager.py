...
class ReactionConfigManager(tkinter.Tk):
    ...
    def __init__(self, mode):
        ...
        add_btn = tkinter.Button(self, text=" + ", font=('Arial', 10), height=10, width=10, image=self.pixel, compound="center", command=self.add_reaction)
        add_btn.grid(column=2, row=1, sticky=tkinter.NSEW)
        del_btn = tkinter.Button(self, text=" - ", font=('Arial', 10), height=10, width=10, image=self.pixel, compound="center", command=self.remove_reaction)
    ...
    def update_container(self, event=None):
        ...
        if mode == "Reaction":
            ...
            for educt, amount in educts.items():
                ...
                del_btn = tkinter.Button(self.container, text="-", font=('Arial', 12), command=lambda p= educt:self.del_educt(p))
        ...
        for product, amount in products.items():
            ...
            entry.grid(column=1, row=i, sticky=tkinter.NSEW)
            del_btn = tkinter.Button(self.container, text="-", font=('Arial', 12), command=lambda p= product:self.del_prod(p))
            del_btn.grid(column=2, row=i, sticky=tkinter.NSEW)
            i -=- 1
        ...
        entry = tkinter.Checkbutton(self.container, text="Unlocked", onvalue=True,offvalue=False, variable=self.string_vars["unlocked"])
    ...
    def on_ok(self):
        json_ob = json.dumps(self.reactions, indent=4)
